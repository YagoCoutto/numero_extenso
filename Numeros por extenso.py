...
while True:
    try:
        num = str(input('Digite um valor entre 0 e 999: '))
        if int(num) < 0:
...
dicionario['20 a 99'] = ['vinte','trinta','quarenta','cinquenta','sessenta','setenta','oitenta','noventa','cem']
dicionario['100 a 900'] = ['cento','duzentos','trezentos','quatrocentos','quinhentos','seiscentos','setecentos','oitocentos','novecentos']
...
if int(num) in range(100, 1000):
    if int(num) == 100:
        ...
        valor1 = int(num[0])-1  
        print(dicionario['100 a 900'][valor1],end=' ')
        valor2 = int(num[1])-2
        if valor2 > 0:
